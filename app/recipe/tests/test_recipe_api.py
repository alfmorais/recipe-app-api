...
class PrivateRecipeAPITests(TestCase):
    ...
    def test_update_user_returns_error(self):
        ...
        new_user = create_user(email="test@example.com", password="test124")
        recipe = create_recipe(user=self.user)
    ...
    def test_update_recipe_assign_tag(self):
        ...
        tag_breakfest = Tag.objects.create(user=self.user, name="breakfast")
        recipe = create_recipe(user=self.user)
        recipe.tag.add(tag_breakfest)
        ...
        self.assertNotIn(tag_breakfest, recipe.tag.all())
    ...
    def test_create_recipe_with_existing_ingredient(self):
        ...
        ingredient = Ingredient.objects.create(user=self.user, name="Lemom")
        payload = {
            "title": "Cauliflower Tacos",
            "time_minutes": 60,
            "price": Decimal("4.30"),
            "ingredients": [{"name": "Lemom"}, {"name": "Salt"}],
            "link": "https://google.com",
        }
        response = self.client.post(RECIPES_URL, payload, format="json")
    ...
    def test_filter_by_ingredients(self):
        ...
        ir1 = Ingredient.objects.create(user=self.user, name="Cheese")
        ir2 = Ingredient.objects.create(user=self.user, name="Chicken")
        r1.ingredients.add(ir1)
        r2.ingredients.add(ir2)
        r3 = create_recipe(user=self.user, title="Black Beans")

        params = {"ingredients": f"{ir1.id}, {ir2.id}"}
        response = self.client.get(RECIPES_URL, params)
